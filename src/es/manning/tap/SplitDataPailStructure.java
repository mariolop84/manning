...
/**
 * A split pail to vertically partition the dataset Clase que representa una
 * estructura Pail que implementa una estrategia para el particionamient
 * vertical del esquema de grafo. Crea un mapa entre los IDs Thrift y la clase
 * el tipo de estructura. PropertyStructure, EdgeStructure
 * SplitDataPailStructure es responsable del directorio raiz del
 * particionamiento vertical, y delega la responsabilidad de los subdirectorios
 * a la clase FieldStructure
 */
public class SplitDataPailStructure extends DataPailStructure {
	...
	static {
		...
		for (DataUnit._Fields k : DataUnit.metaDataMap.keySet()) {
			...
			/*
			 * Properties are identified by the class name of the inspected
			 * object.
			 */
			if (md instanceof StructMetaData && ((StructMetaData) md).structClass.getName().endsWith("Property")) {
				...
				/*
				 * If class name doesnt end with Property, it must bean edge.
				 */
	...
	public boolean isValidTarget(String... dirs) {
		// System.out.println("SplitDataPailStructure.isValidTarget: ...
		// INICIO");
		...
		try {
			...
			/*
			 * The validity check first verifies the DataUnit field ID is in the
			 * field map. Any additional checks are passed to the
			 * FieldStructure.
			 */
			if (s == null)
				return false;
